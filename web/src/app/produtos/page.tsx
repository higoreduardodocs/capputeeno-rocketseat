...
export default function ProdutoPage({ searchParams }: { searchParams: { id: string } }) {
  const { data } = useProduct(searchParams.id)
  
  const handleAddToCart = () => {
    const valueStr = localStorage.getItem('cart-items')
    if (!valueStr)
      return localStorage.setItem('cart-items', JSON.stringify([{ ...data, quantity: 1, id: searchParams.id }]))

    const value = JSON.parse(valueStr)
    const findIndex = value.findIndex((item: {id: string}) => item.id === searchParams.id)
