...
export default function CarrinhoPage() {
  const { value, updateValue } = useLocalStorage<ProductInCart[]>('cart-items', [])
  const cartSubTotal = value.reduce((acc, cur) => acc + (cur.quantity * cur.price_in_cents), 0)
    ...
    <DefaultLayout>
      <WrapperPage>
        <CartDetail>
          <BackButton navigate="/" />
          <h2>Carrinho</h2>
          <p>Total ({value.length} produtos) <span>{formatPrice(cartSubTotal)}</span></p>
          <CartItemList>
            {value.map((item) => (
              <ProductCart key={item.id} cartItem={item} handleUpdateQuantity={handleUpdateCart} handleDelete={handleDelete} />
            ))}
          </CartItemList>
        </CartDetail>
        <CartInfo>
