...
const Button = styled.button<{ isActive: boolean }>`
  ...
  text-align: center;
  border-radius: 8px;
  background-color: ${props => props.isActive ? 'var(--light-color)' : 'var(--shapes-light)'};
  color: ${props => props.isActive ? 'var(--orange-low)' : 'var(--text-dark)'};
  cursor: pointer;
  border: ${props => props.isActive ? '1px solid var(--orange-low)' : '1px solid transparent'};
  ...
  return (
    <Wrapper>
      <WrapperButton isArrow={false}>
        {Array.from({ length: totalButton }, (_, k) => k + 1).map((item) => (
          <Button isActive={page === item} onClick={() => setPage(item)}>{item}</Button>
        ))}
      </WrapperButton>
