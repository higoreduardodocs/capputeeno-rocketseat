...
export default function ProductCart({ cartItem, handleUpdateQuantity, handleDelete }: CartProps) {
  return (
    <CardWrapper>
      ...
      <div>
        ...
        <div>
          <select value={cartItem.quantity} onChange={(ev) => handleUpdateQuantity(cartItem.id, Number(ev.target.value))}>
            {Array
              .from({ length: 5 }, (_, k) => k + 1)
              .map((i) => (
